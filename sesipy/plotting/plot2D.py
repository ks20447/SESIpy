...
class Plot2D:
    ...
    def auto_equal_aspect(self):
        for ax in self.axes.flatten():
            ax.set_aspect("equal")
            
    def auto_grid(self, val):
        for ax in self.axes.flatten():
            ax.grid(val)
    ...
    def axes_set(self):
        ...
        if self.equal_aspect:
            self.auto_equal_aspect()
            
        if not self.grid:
            self.auto_grid(False)

    def show(self, filename=None, auto_set=True):
    ...
    def plot_polygon(
        self, polygon, c="k", fill_outline=False, fill_holes=False, **kwargs
    ):
    ...
    def plot_aoa(self, theta, r, aoa=None, r_min=None, **kwargs):
        ...
        self.ax.plot(theta, r, **kwargs)
        self.ax.set_ylim([r.min() if not r_min else r_min, 0.1])
        ...
        if aoa:
            ...
            self.ax.scatter(
                aoa.theta_peaks, aoa.power_peaks, marker="o", c="g", zorder=10
            )
        ...
        if annotate:
            ...
            for i in range(data.shape[0]):
                for j in range(data.shape[1]):
                    ...
                    text_colour = "white" if data[i, j] < midpoint else "black"
    ...
    def plot_power_metrics_ranked(self, ranked, show_average=False):

        metrics = list(ranked[0]["normalized_metrics"].keys())

        data = np.array(
            [[r["normalized_metrics"][m] for m in metrics] for r in ranked]
        )

        labels = [
            f"Distribution {i + 1} (\\#{r['rank']})"
    ...
    def plot_power_metrics(self, results, show_average=False):
        ...
        colour_data = np.array(
            [[normalized[m][i] for m in metrics] for i in range(len(results))]
        )
        ...
        if show_average:
            ...
            colour_data = np.vstack([
                colour_data,
                np.nanmean(colour_data, axis=0),
            ])
        ...
        self.plot_heatmap(
            ...
            xlabel="Metric",
            ylabel="Power Distribution",
            colorbar_label="Relative performance\n(per metric)",
            vmin=0,
            vmax=1,
        )